...
- Server identity at `initialize`: name `stellar-agent-mcp`, version matching
  the crate's package version (`0.1.0-alpha.8` as of this release).
...
| `accept_mutable_verifier` | bool | no | Opt in to a mutable-admin verifier/policy contract. Rendered as a warning on every approval surface when set. Does not admit an undecodable instance, a non-Wasm executable, or an owner-managed external-reference executable (`sa.contract_instance_unsupported`). |
