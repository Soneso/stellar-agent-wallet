...
| `accept_mutable_verifier` | bool | no | Opt in to a mutable-admin verifier/policy contract. Rendered as a warning on every approval surface when set. Does not admit an undecodable or non-Wasm instance (`sa.contract_instance_unsupported`). |
