...
Two structural rules apply across all surfaces in this alpha. The default network is `stellar:testnet`. Every write or signing command structurally refuses `stellar:mainnet` (wire code `network.mainnet_write_forbidden`): commands that take `--network` refuse before any RPC call or signing, and at the submit layer a declared mainnet network passphrase and a known mainnet RPC URL are each refused at zero RPC cost. Beyond those two, the submit layer asks the endpoint which network it serves, treats that answer rather than the declaration as authoritative, and requires every signature on the envelope to verify under it — so an envelope signed for one network cannot be relayed onto another. `stellar:mainnet` remains accepted for read-only commands.
