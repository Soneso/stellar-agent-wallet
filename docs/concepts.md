...
Which engine runs is selected per profile in `[policy]`. The CLI verbs that move value (`pay`, `claim`, `accounts create` sponsored mode, `trade`, `lend`, `vault`, `trustline`) evaluate policy after the transaction envelope is built and before it is signed, using the same value descriptor their MCP twin derives. `pay` and `claim` gate their default (build-sign-submit) and `--build-only` invocations this way; `--sign-only` and `--submit-only` operate on an already-built, opaque XDR envelope and are not separately gated. `accounts create`'s Friendbot mode is never gated — it debits no wallet-held funds. `pay`, `claim`, and `accounts create` take a `--profile` flag; with no persisted `<name>.toml` profile file, they synthesize an in-memory `Noop`-engine testnet profile rather than requiring one to be authored up front.
