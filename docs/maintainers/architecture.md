...
The same tracker also backs the DeFi adapter submit paths (`stellar_dex_trade`, `stellar_blend_lend`, `stellar_defindex_vault_deposit`/`_withdraw`), via a `SequenceFloorHook` trait (`stellar-agent-network::sequence_floor`) that decouples the tracker's concrete type from the lower crates in the dependency graph. Each MCP DeFi tool wraps `WalletServer`'s tracker in a local `TrackerHook` and sets it on `DefiAdapterCtx::sequence_floor`, which the adapter crates (`stellar-agent-blend`, `stellar-agent-dex`, `stellar-agent-defindex`) forward into `SubmitInvokeArgs::sequence_floor` at their `submit_signed_invoke` call site (`stellar-agent-smart-account::submit`). `submit_signed_invoke`'s own account fetch consults the SAME hook for the bounded catch-up poll, and records the consumed sequence back into it on confirmed submit — one shared floor per source account across the classic commit verbs and the DeFi adapters, not two independent ones. `SubmitInvokeArgs::sequence_floor` defaults to `None` for every other caller of `submit_signed_invoke` (rules/signers self-calls, the multicall bundle path), which is CLI-only and out of scope for the same reason the tracker itself is MCP-server-only.
