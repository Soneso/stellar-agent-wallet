...
The repository is a single Cargo workspace (`resolver = "2"`) with 35 member crates under `crates/`, all named `stellar-agent-*`. Shared package metadata is inherited from `[workspace.package]`: version `0.1.0-alpha.6`, Rust edition `2024`, license `Apache-2.0`, repository `https://github.com/Soneso/stellar-agent-wallet`, author `Soneso`. Every member crate is published on crates.io at the shared workspace version, in dependency order from the tagged release commit.
...
`stellar-agent-defi` is the adapter substrate: the contract-pin framework, the typed-preview trait, and the dispatch-verb seam (`lend`/`trade`/`vault`/`bridge`). It ships no live verb. The adapters `stellar-agent-defindex`, `stellar-agent-dex`, and `stellar-agent-stablecoin` implement that substrate and register the `vault`, `trade`/`quote`, and `trustline` verbs; the adapters whose submit path runs through a smart account also depend on `smart-account`.
...
| `stellar-agent-loopback-http` | Shared tower/axum defence-in-depth middleware for the wallet's browser-facing HTTP listeners (Host and Origin allowlists, hardened security headers), plus the `brand` module holding the stylesheet, the inline project mark, and `PageIdentity` (the optional per-deployment name and mark those listeners' pages render), and the `escape` module holding the one HTML-escaping definition every page applies to dynamic text. |
...
- **Plain libraries:** `core`, `network`, `claimable`, `sep5`, `xdr-limits`, `nonce`, `headless-keyring`, `smart-account`, `webauthn-bridge`, `loopback-http`, `approval-ui`, `approval-remote`, `pool`, `sep7`, `sep10`, `sep43`, `sep45`, `sep48`, `sep53`, `anchor`, `toolsets`, `toolsets-install`, `toolsets-runtime`, `defi`, `defindex`, `dex`, `stablecoin`, `x402`, `x402-identity`, `mpp`.
...
The MCP server registers tools at link time. Each tool function inside the `#[mcp_tool_router]` impl block carries an `#[mcp_tool_item(...)]` annotation; the `mcp-macros` proc-macro emits an `inventory::submit!` record per tool. `WalletServer::new` iterates `inventory::iter::<McpToolRegistration>()` to build the descriptor map, and the same map is what `PolicyEngine::evaluate` is called against at `tools/call` dispatch. The policy-engine call site at dispatch is identical whether the active engine is `NoopPolicyEngine` or `PolicyEngineV1`; only the resolved engine differs per profile. The bridge from the network account view to the policy engine's `AccountReservesView` / `AccountIdentityView` lives in `stellar-agent-network::policy_view` (`AccountViewAdapter`) — hosted there rather than in `core` or `mcp` because `AccountView` is defined in `network`, which already depends on `core`, so no new dependency edge is needed; both the MCP tool handlers and the CLI's classic verbs (`pay`, `claim`, `accounts create`) construct the adapter directly when populating the evaluation context. `stellar-agent-mcp::policy_adapter` re-exports `AccountViewAdapter` from its former path for compatibility. The MCP transport is a bounded stdio codec with a 1 MiB maximum line length, set explicitly rather than using the framework default.

`WalletServer` holds a small amount of in-process, long-lived state alongside the tool registry, each behind its own `TokioMutex` for async-safe access across concurrent dispatches: the nonce replay window (`stellar-agent-nonce::ReplayWindow`), and `stellar-agent-mcp::sequence_floor::SequenceFloorTracker` — a bounded, FIFO-evicted map from source account to the highest sequence number a confirmed submit in this process consumed. The `stellar_pay_commit` / `stellar_claim_commit` / `stellar_trustline_commit` / `stellar_create_account_commit` build paths (and their simulate-phase twins) consult the tracker after every `fetch_account`: a fetched sequence below the tracked floor triggers a bounded re-poll before the build proceeds, closing an ordinary read-after-write propagation-lag window between the RPC node that served a just-confirmed submit and the node that serves the next build. The tracker is advisory only — a build that still observes a stale sequence after the bounded window proceeds with it, and the eventual submit fails typed `submission.sequence_number_stale` exactly as it would without the tracker. It is MCP-server-only: the CLI is a fresh process per invocation with no history to track against.

The same tracker also backs the DeFi adapter submit paths (`stellar_dex_trade`, `stellar_defindex_vault_deposit`/`_withdraw`), via a `SequenceFloorHook` trait (`stellar-agent-network::sequence_floor`) that decouples the tracker's concrete type from the lower crates in the dependency graph. Each MCP DeFi tool wraps `WalletServer`'s tracker in a local `TrackerHook` and sets it on `DefiAdapterCtx::sequence_floor`, which the adapter crates (`stellar-agent-dex`, `stellar-agent-defindex`) forward into `SubmitInvokeArgs::sequence_floor` at their `submit_signed_invoke` call site (`stellar-agent-smart-account::submit`). `submit_signed_invoke`'s own account fetch consults the SAME hook for the bounded catch-up poll, and records the consumed sequence back into it on confirmed submit — one shared floor per source account across the classic commit verbs and the DeFi adapters, not two independent ones. `SubmitInvokeArgs::sequence_floor` defaults to `None` for every other caller of `submit_signed_invoke` (rules/signers self-calls, the multicall bundle path), which is CLI-only and out of scope for the same reason the tracker itself is MCP-server-only.
