# CLI reference: wallet (smart-account governance)

The `wallet` command group governs an on-chain OpenZeppelin smart-account: its context rules, its signer sets and thresholds, the policy contracts attached to each rule, and the supporting infrastructure (verifier registry, multicall router registry, upgrade timelock). It also submits multicall bundles through the registered router.

Most on-chain signing verbs that mutate context-rule, signer, or timelock state structurally refuse `mainnet` before any RPC call or signing key access, surfacing the wire code `network.mainnet_write_forbidden`: the `wallet rules` write verbs, all `wallet signers` verbs (including `list` and `refresh`, which emit audit rows), the timelock write verbs (`schedule`, `cancel`, `execute`), and `wallet sa deploy-webauthn-verifier`. The exceptions:

- `wallet sa migrate-verifier` allows a mainnet dry-run and permits a mainnet submit only when `--confirm-mainnet-migrate` is supplied; it never returns `network.mainnet_write_forbidden`.
- `wallet multicall` accepts `mainnet` at the flag level but requires a multicall router registered for mainnet.
- `wallet sa register-multicall` / `wallet sa unregister-multicall` accept `mainnet` as a local-registry key.
- The read-only verbs (`wallet rules get`, `wallet rules verify-pins`, `wallet rules list` / `wallet sa list-rules`, `wallet sa list-verifiers`, `wallet sa timelock list-pending`) accept `mainnet` unconditionally.
...
- `--signer-webauthn <CREDENTIAL_NAME>` — a passkey signer, resolved from the profile's passkey registry (see [`credentials add-passkey`](profile-and-governance.md)). Repeatable. The verifier contract address is read from the verifier registry, which is populated by `wallet sa deploy-webauthn-verifier`.
...
```bash
stellar-agent wallet rules get \
  --account CABC...WXYZ \
  --rule-id 1 \
...
```bash
stellar-agent wallet rules set-name \
  --account CABC...WXYZ \
  --rule-id 1 \
  --name treasury \
  --signer-secret-env WALLET_SK
...
```bash
stellar-agent wallet rules set-valid-until \
  --account CABC...WXYZ \
  --rule-id 1 \
  --valid-until none \
  --signer-secret-env WALLET_SK
...
```bash
stellar-agent wallet rules delete \
  --account CABC...WXYZ \
  --rule-id 1 \
  --signer-secret-env WALLET_SK
```
...
```bash
stellar-agent wallet rules verify-pins \
  --account CABC...WXYZ \
  --rule-id 1 \
  --signer-secret-env WALLET_SK
```
...
```bash
stellar-agent wallet rules add-policy \
  --account CABC...WXYZ \
  --rule-id 1 \
...
```bash
stellar-agent wallet rules remove-policy \
  --account CABC...WXYZ \
  --rule-id 1 \
  --policy-id 0 \
  --signer-secret-env WALLET_SK
...
Enumerates the active context rules on a smart-account via on-chain scan. Read-only; no signing. `mainnet` is accepted. This is the canonical name for the enumeration; it produces the same JSON envelope as `wallet sa list-rules` and takes the same flags (see [`wallet sa list-rules`](#wallet-sa-list-rules)).

```bash
stellar-agent wallet rules list --account CABC...WXYZ
```
...
```bash
stellar-agent wallet signers list \
  --account CABC...WXYZ \
  --rule-id 0 \
  --signer-secret-env WALLET_SK
```

### `wallet signers refresh`
...
```bash
stellar-agent wallet signers refresh \
  --account CABC...WXYZ \
  --rule-id 0 \
  --signer-secret-env WALLET_SK
```
...
```bash
stellar-agent wallet signers add \
  --account CABC...WXYZ \
  --rule-id 0 \
...
- `--signer-id <U32>` (required) — the on-chain signer id to remove, from `wallet signers list`.

```bash
stellar-agent wallet signers remove \
  --account CABC...WXYZ \
  --rule-id 0 \
  --signer-id 2 \
  --signer-secret-env WALLET_SK
...
```bash
stellar-agent wallet signers set-threshold \
  --account CABC...WXYZ \
  --rule-id 0 \
  --new-threshold 2 \
  --signer-secret-env WALLET_SK
...
Deploy-time, registry-management, and migration operations that sit beneath the context-rule lifecycle.
...
```bash
stellar-agent wallet sa list-verifiers --output table
```

### `wallet sa list-rules`

Enumerates the active context rules on a smart-account by scanning the on-chain `[0, max_scan_id)` rule-id space and returning each active rule in `rule_id` order. Read-only; no signing. `mainnet` is accepted. This is the alias backing `wallet rules list`; both produce the same envelope.
...
```bash
stellar-agent wallet sa list-rules --account CABC...WXYZ
```

### `wallet sa register-multicall`
...
```bash
stellar-agent wallet sa register-multicall \
  --address CRTR...WXYZ \
  --wasm-sha256 67800690...b27a
```

### `wallet sa unregister-multicall`
...
```bash
stellar-agent wallet sa unregister-multicall --network testnet
```
...
#### `wallet sa timelock list-pending`
...
```bash
stellar-agent wallet sa timelock list-pending --timelock CTLCK...WXYZ
```
