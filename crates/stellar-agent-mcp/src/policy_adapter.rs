...
//! # Why this lives here
//!
//! `stellar-agent-network` already depends on `stellar-agent-core`
//! (`AccountView` is defined there).  Placing blanket impls of
//! `AccountReservesView` / `AccountIdentityView` for `AccountView` inside
//! `stellar-agent-core` would therefore create a circular dependency.
//! `stellar-agent-mcp` already carries **both** crates as direct
//! dependencies, making it the correct home for the bridge adapter.
//!
//! # Trait split
//!
...
//! # Per-tool wiring
//!
//! `stellar_pay` and `stellar_create_account` fetch the source account and pass
//! this adapter as `account_view` (consumed by the `minimum_reserve`
//! criterion).  `stellar_pay` additionally fetches the destination account and
//! passes its adapter as `identity_view` (consumed by `home_domain_resolved`);
//! `stellar_create_account` passes `None` for `identity_view` because the
//! destination does not yet exist.  Both call `dispatch_gate_with_views`.
//!
//! Tools that do not fetch an account call the plain `dispatch_gate`, which
//! passes `None` for both views — so a V1 policy that configures
//! `minimum_reserve` or `home_domain_resolved` for such a tool fails closed
//! (the criterion returns `PolicyError::CriterionEvaluationFailed` when its
//! view is absent, denying rather than allowing the call).
...
/// Adapts a `stellar_agent_network::AccountView` reference to the
...
/// criterion is active.  The tool handler must have already fetched the
/// `AccountView` from the network before constructing the context.
///
/// # Per-tool wiring
///
/// The actual call-site population (tool handlers constructing this adapter
/// and passing it in) is a separate concern.  See the module-level note.
///
/// # Examples
///
/// ```
/// use stellar_agent_mcp::policy_adapter::AccountViewAdapter;
...
mod tests {
    ...
    use super::*;
    use stellar_agent_core::BASE_RESERVE_STROOPS;
    use stellar_agent_network::{AssetView, BalanceView, ThresholdsView};
