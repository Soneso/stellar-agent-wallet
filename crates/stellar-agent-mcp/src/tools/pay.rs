...
impl WalletServer {
    ...
    pub(crate) async fn stellar_pay_commit_impl(
        ...
        let mut authoritative_args =
            match decode_authoritative_args(&args.envelope_xdr, "stellar_pay_commit") {
                Ok(a) => a,
                Err(e) => {
                    tracing::debug!(
                        error = %e,
        ...
        {
            let amount_stroops: i64 = match authoritative_args
                .get("amount_stroops")
                .and_then(crate::tools::amount_wire::value_as_stroops_i64)
            {
                ...
                None => {
                    return Ok(crate::tools::common::business_error_result(
                        "simulation.divergence",
                        "authoritative_args missing required `amount_stroops` field",
                    ));
                }
            };
            ...
            if let Err(result) = high_value_cross_check(
                ...
                |oracle_client| {
                    ...
                    let oracle_asset = oracle_asset.clone();
                    let oracle_memo = oracle_memo.clone();
                    ...
                    async move {
                        ...
                        oracle_builder
                            ...
                            .map_err(|e| {
                                rmcp::ErrorData::internal_error(
                                    format!("oracle rebuild failed: {e}"),
                                    None,
                                )
                            })?;
                        oracle_builder.memo(&oracle_memo).map_err(|e| {
                            rmcp::ErrorData::internal_error(
                                format!("oracle rebuild failed: {e}"),
                                None,
                            )
        ...
        let recorder = match crate::tools::submission_record::build_recorder(
            crate::tools::submission_record::CommitRecord {
                ...
                descriptor: self.tool_registry.get("stellar_pay_commit"),
                value_class,
            ...
            let authoritative =
                match decode_authoritative_args(&args.envelope_xdr, "stellar_pay_commit") {
                    ...
                    Err(e) => {
                        return Ok(crate::tools::common::business_error_result(
                            "simulation.divergence",
                            format!("toolset_gated envelope_xdr decode: {e}"),
                        ));
                    }
                };
