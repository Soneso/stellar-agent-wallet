...
//! # Policy evaluation
//!
//! The MCP tool runs `WalletServer::dispatch_gate` (which evaluates the
//! operator `PolicyEngineV1` document: Deny / RequireApproval / chain_id /
//! counterparty) before signing.  The CLI `lend` subcommand mirrors this by
...
impl WalletServer {
    ...
    async fn stellar_blend_lend(
        ...
        // ── Policy gate (dispatch_gate prerequisite) ──────────────────────────
        let args_value = json!({
