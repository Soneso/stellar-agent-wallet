...
/// Builds one [`ValueLeg`] per `reqs` entry for the `stellar_blend_lend`
...
///
/// The four liquidation discriminants (6-9) are unreachable via
/// `stellar_blend_lend` — the tool refuses request types > 5 before this
/// function runs. They map to the non-debit shape deliberately: a fill
/// request's `req.amount` is a fill PERCENTAGE, not a token debit in stroops,
/// so it must never be summed by a value cap. Whoever wires a `liquidate` verb
/// must model the auction's actual token movement explicitly rather than reuse
/// this mapping.
///
...
///
/// Called from `stellar_blend_lend` with the SAME `blend_requests` vector
/// that is later placed into `LendArgs` and signed — the single-decode
/// invariant. This is a pure function so it can be unit-tested directly
/// without exercising the async handler.
#[must_use]
fn blend_value_legs(reqs: &[BlendRequest], pool: &str) -> Vec<ValueLeg> {
    reqs.iter()
        .map(|req| {
            let (kind, amount) = match req.request_type {
                ...
                // Liquidation verbs (6-9) are unreachable here (the tool refuses
                // request types > 5). `req.amount` for a fill is a PERCENTAGE,
                // not a stroops debit, so these map to the non-debit shape and
                // must never be summed by a value cap; a future `liquidate` verb
                // must model the auction's token movement explicitly.
                ...
                // `RequestType` is `#[non_exhaustive]`: a future discriminant
                // falls back to the conservative non-debit shape rather than
                // guessing at a future protocol version's direction.
                _ => (ActionKind::LendWithdraw, None),
            };
...
mod tests {
    ...
    // ── blend_value_legs: value-carrying policy-gate leg construction ─────────
    ...
    fn blend_value_legs_maps_outflow_kinds_to_lend_with_matching_amount_and_asset() {
        ...
        for rt in [
            ...
            let legs = super::blend_value_legs(std::slice::from_ref(&req), POOL);
    ...
    fn blend_value_legs_mixed_request_call_yields_correct_direction_per_leg() {
        ...
        let legs = super::blend_value_legs(&reqs, POOL);
