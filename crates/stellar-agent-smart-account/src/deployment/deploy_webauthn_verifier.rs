//! WebAuthn-verifier contract deployment via Soroban `CreateContractV2`.
//!
//! Deploys the vendored OZ `multisig_webauthn_verifier_example.wasm` to
//! a Stellar network and records the resulting contract address in the wallet-local
//! [`VerifierRegistry`] (`~/.config/stellar-agent/networks.toml`).
//!
//! # Deploy flow
//!
//! 1. **Runtime SHA gate**: re-verify
//!    `sha256(WEBAUTHN_VERIFIER_WASM) == WEBAUTHN_VERIFIER_WASM_SHA256` before
//!    any submission.  Refuses with [`SaError::WebAuthnVerifierProvenanceMismatch`]
//!    on mismatch.  The `include_bytes!`-baked `&'static [u8]` is the same slice
//!    passed to `UploadContractWasm`; no TOCTOU window exists.
//! 2. **Idempotency check**: open the [`VerifierRegistry`]; if an entry already exists
//!    for the target network with the same `wasm_sha256`, return immediately with
...
/// Deterministic-salt domain-prefix string for the WebAuthn-verifier deploy.
///
...
/// Arguments for [`deploy_webauthn_verifier`].
pub struct WebAuthnVerifierDeployArgs {
...
/// Result of a successful [`deploy_webauthn_verifier`] call.
#[derive(Debug, Clone, serde::Serialize)]
#[non_exhaustive]
pub struct WebAuthnVerifierDeployResult {
...
/// Deploys the vendored OZ WebAuthn-verifier contract and records the address in
...
/// # Errors
///
/// - [`SaError::WebAuthnVerifierProvenanceMismatch`] — runtime SHA gate failed.
/// - [`SaError::NetworksTomlIo`] — registry read/write failed.
/// - [`SaError::NetworksTomlParse`] — registry TOML invalid.
/// - [`SaError::WebAuthnVerifierSha256Drift`] — registry has different sha256 for network.
...
/// WASM bytes do not match `WEBAUTHN_VERIFIER_WASM_SHA256`.
pub async fn deploy_webauthn_verifier(
    args: WebAuthnVerifierDeployArgs,
    audit_writer: Option<&mut AuditWriter>,
) -> Result<WebAuthnVerifierDeployResult, SaError> {
    let chain_id = caip2_chain_id_for_passphrase(&args.network_passphrase);
    let is_dry_run = args.dry_run;

    let outcome = deploy_webauthn_verifier_body(args).await;
    ...
    if let Some(writer) = audit_writer {
        ...
        // Emit SaRawInvocation for the deploy_webauthn_verifier operation.
        ...
        if let Err(e) = writer.write_entry(ra_entry) {
            tracing::warn!(
                error = %e,
                "deploy_webauthn_verifier: SaRawInvocation audit write failed"
            );
        }
    }
...
async fn deploy_webauthn_verifier_body(
    args: WebAuthnVerifierDeployArgs,
) -> Result<WebAuthnVerifierDeployResult, SaError> {
    ...
    // from deploy.rs (the cargo test compile-time gate in webauthn_verifier.rs is the
    // first line of defence).
    #[cfg(debug_assertions)]
    {
        let mut h = Sha256::new();
        h.update(WEBAUTHN_VERIFIER_WASM);
    ...
    }
    {
        ...
        if actual != WEBAUTHN_VERIFIER_WASM_SHA256 {
            return Err(SaError::WebAuthnVerifierProvenanceMismatch {
                expected: WEBAUTHN_VERIFIER_WASM_SHA256.to_owned(),
                actual,
            });
        }
    }
    ...
    let wasm_hash_bytes: [u8; 32] =
        decode_hex32(WEBAUTHN_VERIFIER_WASM_SHA256).map_err(|()| SaError::DeploymentFailed {
            phase: "build",
            redacted_reason: "WEBAUTHN_VERIFIER_WASM_SHA256 const is not valid 64-char hex"
                .to_owned(),
        })?;
    ...
    // ── Dry-run path ──────────────────────────────────────────────────────────
    if args.dry_run {
        return Ok(WebAuthnVerifierDeployResult {
            verifier_address: derived_verifier_address,
            verifier_wasm_sha256: WEBAUTHN_VERIFIER_WASM_SHA256.to_owned(),
    ...
    if let Some(existing) = registry.webauthn_verifier_for(&args.network_passphrase) {
        if existing.wasm_sha256 == WEBAUTHN_VERIFIER_WASM_SHA256 {
            info!(
                verifier = %stellar_agent_core::observability::redact_strkey_first5_last5(
                    &existing.address),
                network = &args.network_passphrase,
                "deploy_webauthn_verifier: already deployed; returning existing entry"
            );
            return Ok(WebAuthnVerifierDeployResult {
        ...
        return Err(SaError::WebAuthnVerifierSha256Drift {
            ...
            attempted: WEBAUTHN_VERIFIER_WASM_SHA256.to_owned(),
        });
    }
    ...
    let upload_tx_hash: Option<String> = if wasm_already_on_chain {
        info!(
            wasm_hash = %redact_wasm_hash(WEBAUTHN_VERIFIER_WASM_SHA256),
            "deploy_webauthn_verifier: WASM already on-chain; skipping upload"
        );
        None
    } else {
        info!(
            wasm_hash = %redact_wasm_hash(WEBAUTHN_VERIFIER_WASM_SHA256),
            "deploy_webauthn_verifier: uploading WASM"
        );
        ...
        info!(
            ...
            "deploy_webauthn_verifier: WASM uploaded"
        );
    ...
    info!(
        ...
        "deploy_webauthn_verifier: deployment verified"
    );

    // ── Step 13: Record in registry ───────────────────────────────────────────
    let _ = registry.record_webauthn_verifier(
        &args.network_passphrase,
    ...
    Ok(WebAuthnVerifierDeployResult {
        verifier_address: derived_verifier_address,
        verifier_wasm_sha256: WEBAUTHN_VERIFIER_WASM_SHA256.to_owned(),
        network_passphrase: args.network_passphrase,
...
mod tests {
    ...
    ///
    /// Uses the same logic as the gate in `deploy_webauthn_verifier_body` to
