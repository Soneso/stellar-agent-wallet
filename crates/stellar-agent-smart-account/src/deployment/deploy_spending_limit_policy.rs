//! Spending-limit-policy contract deployment via Soroban `CreateContractV2`.
//!
...
//!
//! The OZ policy keys all state by
//! `SpendingLimitStorageKey::AccountContext(smart_account, context_rule_id)`
//! (`packages/accounts/src/policies/spending_limit.rs:145-147`, SHA `a9c4216`),
//! so one deployed instance serves every account and every context rule on the
//! network.  The wallet deploys exactly one per network and re-uses it for every
//! typed `add-policy --kind spending-limit` install.
//!
//! # Deploy flow
//!
//! 1. **Runtime SHA gate**: re-verify
//!    `sha256(SPENDING_LIMIT_POLICY_WASM) == SPENDING_LIMIT_POLICY_WASM_SHA256`
//!    before any submission.  Refuses with
//!    [`SaError::SpendingLimitPolicyProvenanceMismatch`] on mismatch.  The
//!    `include_bytes!`-baked `&'static [u8]` is the same slice passed to
//!    `UploadContractWasm`; no TOCTOU window exists.
//! 2. **Idempotency check**: open the [`VerifierRegistry`]; if a
//!    spending-limit-policy entry already exists for the target network with the
//!    same `wasm_sha256`, return immediately with `status: "already_deployed"` and
//!    no RPC traffic.
//! 3. **Deploy**: `UploadContractWasm` (conditional on WASM not already on-chain) +
//!    `CreateContractV2(args: [])`.  The policy has no `__constructor`; no
//!    constructor args required.  Confirmed from OZ
//!    `examples/multisig-smart-account/spending-limit-policy/src/contract.rs`
//!    (SHA `a9c4216`) and `vendor/oz-spending-limit-policy/v0.7.2/PROVENANCE.md`,
//!    which export only the `Policy` trait methods (`enforce`, `install`,
//!    `uninstall`, `get_spending_limit_data`, `set_spending_limit`).
//! 4. **Record**: invoke [`VerifierRegistry::record_spending_limit_policy`] + persist.
//! 5. **Audit-log**: emit `SaRawInvocation` with `operation: "deploy_spending_limit_policy"`.
//!
//! # Deterministic salt
//!
//! Salt = `SHA256("oz-spending-limit-policy-v0.7.2-" || network_passphrase)`.
...
use crate::spending_limit_policy::{SPENDING_LIMIT_POLICY_WASM, SPENDING_LIMIT_POLICY_WASM_SHA256};
use crate::verifiers::VerifierRegistry;

/// Deterministic-salt domain-prefix string for the spending-limit-policy deploy.
///
/// The deterministic salt is `SHA256(POLICY_SALT_DOMAIN_PREFIX || network_passphrase)`.
/// The version suffix in the prefix pins the salt to the OZ WASM version, so a
/// future policy bump produces a different address even on the same network with
/// the same deployer.  Pinned to a single `const` so a version bump touches one
/// site.
const POLICY_SALT_DOMAIN_PREFIX: &str = "oz-spending-limit-policy-v0.7.2-";

// ── Public types ──────────────────────────────────────────────────────────────

/// Arguments for [`deploy_spending_limit_policy`].
pub struct SpendingLimitPolicyDeployArgs {
...
/// Result of a successful [`deploy_spending_limit_policy`] call.
#[derive(Debug, Clone, serde::Serialize)]
#[non_exhaustive]
pub struct SpendingLimitPolicyDeployResult {
...
/// Deploys the vendored OZ spending-limit-policy contract and records the address
/// in the wallet-local [`VerifierRegistry`].
///
/// See the module-level documentation for the full deploy flow.
///
/// # Errors
///
/// - [`SaError::SpendingLimitPolicyProvenanceMismatch`] — runtime SHA gate failed.
/// - [`SaError::NetworksTomlIo`] — registry read/write failed.
/// - [`SaError::NetworksTomlParse`] — registry TOML invalid.
/// - [`SaError::SpendingLimitPolicySha256Drift`] — registry has different sha256 for network.
...
/// WASM bytes do not match `SPENDING_LIMIT_POLICY_WASM_SHA256`.
pub async fn deploy_spending_limit_policy(
    args: SpendingLimitPolicyDeployArgs,
    audit_writer: Option<&mut AuditWriter>,
) -> Result<SpendingLimitPolicyDeployResult, SaError> {
    let chain_id = caip2_chain_id_for_passphrase(&args.network_passphrase);
    let is_dry_run = args.dry_run;

    let outcome = deploy_spending_limit_policy_body(args).await;
    ...
    if let Some(writer) = audit_writer {
        ...
        // Emit SaRawInvocation for the deploy_spending_limit_policy operation.
        ...
        if let Err(e) = writer.write_entry(ra_entry) {
            tracing::warn!(
                error = %e,
                "deploy_spending_limit_policy: SaRawInvocation audit write failed"
            );
        }
    }
...
async fn deploy_spending_limit_policy_body(
    args: SpendingLimitPolicyDeployArgs,
) -> Result<SpendingLimitPolicyDeployResult, SaError> {
    ...
    // from deploy.rs (the cargo test compile-time gate in spending_limit_policy.rs is the
    // first line of defence).
    #[cfg(debug_assertions)]
    {
        let mut h = Sha256::new();
        h.update(SPENDING_LIMIT_POLICY_WASM);
        let observed = to_hex(&h.finalize());
        debug_assert_eq!(
            observed, SPENDING_LIMIT_POLICY_WASM_SHA256,
            "SPENDING_LIMIT_POLICY_WASM bytes do not match the compile-time pin; \
             re-run vendor/oz-spending-limit-policy/v0.7.2/build.sh"
        );
    }
    {
        let mut h = Sha256::new();
        ...
        if actual != SPENDING_LIMIT_POLICY_WASM_SHA256 {
            return Err(SaError::SpendingLimitPolicyProvenanceMismatch {
                expected: SPENDING_LIMIT_POLICY_WASM_SHA256.to_owned(),
                actual,
            });
        }
    }
    ...
    let wasm_hash_bytes: [u8; 32] =
        decode_hex32(SPENDING_LIMIT_POLICY_WASM_SHA256).map_err(|()| {
    ...
    //
    // Salt = SHA256(POLICY_SALT_DOMAIN_PREFIX || network_passphrase).
    ...
    let salt_input = format!("{POLICY_SALT_DOMAIN_PREFIX}{}", args.network_passphrase);
    ...
    // ── Dry-run path ──────────────────────────────────────────────────────────
    if args.dry_run {
        return Ok(SpendingLimitPolicyDeployResult {
            policy_address: derived_policy_address,
            policy_wasm_sha256: SPENDING_LIMIT_POLICY_WASM_SHA256.to_owned(),
    ...
    if let Some(existing) = registry.spending_limit_policy_for(&args.network_passphrase) {
        if existing.wasm_sha256 == SPENDING_LIMIT_POLICY_WASM_SHA256 {
            info!(
                policy = %stellar_agent_core::observability::redact_strkey_first5_last5(
                    &existing.address),
                network = &args.network_passphrase,
                "deploy_spending_limit_policy: already deployed; returning existing entry"
            );
            return Ok(SpendingLimitPolicyDeployResult {
                policy_address: existing.address.clone(),
                policy_wasm_sha256: existing.wasm_sha256.clone(),
                network_passphrase: args.network_passphrase,
                tx_hash: None,
                ledger: None,
        ...
        return Err(SaError::SpendingLimitPolicySha256Drift {
            ...
            attempted: SPENDING_LIMIT_POLICY_WASM_SHA256.to_owned(),
        });
    }
    ...
    let upload_tx_hash: Option<String> = if wasm_already_on_chain {
        info!(
            wasm_hash = %redact_wasm_hash(SPENDING_LIMIT_POLICY_WASM_SHA256),
            "deploy_spending_limit_policy: WASM already on-chain; skipping upload"
        );
        None
    } else {
        info!(
            wasm_hash = %redact_wasm_hash(SPENDING_LIMIT_POLICY_WASM_SHA256),
            "deploy_spending_limit_policy: uploading WASM"
        );

        let wasm_bytes: BytesM = SPENDING_LIMIT_POLICY_WASM
            .to_vec()
            .try_into()
        ...
        info!(
            ...
            "deploy_spending_limit_policy: WASM uploaded"
        );
    ...
    //
    // The OZ spending-limit policy has NO `__constructor` — it exports only the
    // `Policy` trait methods (`enforce`, `install`, `uninstall`,
    // `get_spending_limit_data`, `set_spending_limit`).  Constructor args must be
    // empty.  Cross-check: OZ
    // `examples/multisig-smart-account/spending-limit-policy/src/contract.rs`
    // (SHA `a9c4216`) and `vendor/oz-spending-limit-policy/v0.7.2/PROVENANCE.md`.
    let deployer_pk =
    ...
    // Empty constructor args for the policy contract.
    ...
    info!(
        ...
        wasm_hash = %redact_wasm_hash(SPENDING_LIMIT_POLICY_WASM_SHA256),
        "deploy_spending_limit_policy: submitting deploy transaction"
    );
    ...
    info!(
        ...
        "deploy_spending_limit_policy: deployment verified"
    );

    // ── Step 13: Record in registry ───────────────────────────────────────────
    let _ = registry.record_spending_limit_policy(
        &args.network_passphrase,
        derived_policy_address.clone(),
        SPENDING_LIMIT_POLICY_WASM_SHA256.to_owned(),
    )?;
    registry.persist()?;
    ...
    Ok(SpendingLimitPolicyDeployResult {
        policy_address: derived_policy_address,
        policy_wasm_sha256: SPENDING_LIMIT_POLICY_WASM_SHA256.to_owned(),
        network_passphrase: args.network_passphrase,
...
mod tests {
    ...
    use crate::spending_limit_policy::{
        SPENDING_LIMIT_POLICY_WASM, SPENDING_LIMIT_POLICY_WASM_SHA256,
    };
    ...
    ///
    /// Uses the same logic as the gate in `deploy_spending_limit_policy_body` to
    ...
    fn deterministic_salt_is_stable_and_differs_by_passphrase() {
        ...
        let domain_testnet = format!("{POLICY_SALT_DOMAIN_PREFIX}{testnet}");
        let domain_mainnet = format!("{POLICY_SALT_DOMAIN_PREFIX}{mainnet}");
    ...
    /// Asserts the spending-limit-policy salt domain prefix differs from both
    /// verifier prefixes, so the policy derives a distinct address from the
    /// verifiers for the same deployer + network.
    #[test]
    fn salt_domain_prefix_differs_from_verifiers() {
        let passphrase = "Test SDF Network ; September 2015";
        let policy_domain = format!("{POLICY_SALT_DOMAIN_PREFIX}{passphrase}");
        let ed25519_domain = format!("oz-ed25519-verifier-v0.7.2-{passphrase}");
        let webauthn_domain = format!("oz-webauthn-verifier-v0.7.2-{passphrase}");

        let policy_salt: [u8; 32] = Sha256::digest(policy_domain.as_bytes()).into();
        ...
        assert_ne!(
            policy_salt, ed25519_salt,
            "policy vs ed25519 salt must differ"
        );
        assert_ne!(
            policy_salt, webauthn_salt,
            "policy vs webauthn salt must differ"
        );
    }
}
