...
/// 5. Spawn `target/release/stellar-agent smart-account signers add` with the 16th
...
async fn h1_16th_signer_refused() {
    ...
    eprintln!(
        "h1: invoking binary: smart-account signers add --account {} --rule-id {} \
    ...
    let output = std::process::Command::new(&binary)
        ...
        .output()
        .expect("spawn smart-account signers add");
    ...
    assert_ne!(
        ...
        0,
        "smart-account signers add must exit non-zero when adding the 16th signer; \
         stdout: {stdout}"
    );
...
async fn h2_6th_policy_refused() {
    ...
    eprintln!(
        "h2: invoking binary: smart-account rules add-policy --account {} --rule-id {} \
    ...
    let output = std::process::Command::new(&binary)
        ...
        .env(signer_env_var, &operator_s_strkey)
        .output()
        .expect("spawn smart-account rules add-policy");
    ...
    assert_ne!(
        output.status.code().unwrap_or(0),
        0,
        "smart-account rules add-policy must exit non-zero when adding the 6th policy; \
         stdout: {stdout}"
    );
