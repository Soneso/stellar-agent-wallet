...
//!
//! ```text
//! cargo test --features testnet-integration --test wallet_rules_testnet_acceptance
//! ```
//!
...
/// `wallet_session_rule_horizon_testnet_acceptance.rs`.  Tests that need to
...
async fn full_metadata_only_lifecycle_on_testnet() {
    ...
    // decoding of the renamed `name` is deferred to the `wallet rules list`
    ...
    // `wallet_session_rule_horizon_testnet_acceptance.rs`; here we use an
...
/// would be emitted for every `wallet rules` CLI operation — a forensic-evidence
/// gap.
///
