...
//! executable is a CAP-85 external reference whose owner holds no live tag
//! entry.  Such a contract has no hash the wallet can pin: the signing-time
//! drift check could not observe a change to its code.
...
/// A verifier or policy whose instance executable is an external reference
...
async fn external_ref_without_live_tag_entry_rejected_before_any_override() {
    ...
    for contract_kind in [ContractKind::Verifier, ContractKind::Policy] {
        for (accept_mutable_verifier, accept_unknown_verifier) in
            [(false, false), (true, false), (false, true), (true, true)]
        {
            // The instance names the owner's tag, and the owner holds no live
            // entry under it.
            ...
            let definition = ContextRuleDefinition::new(
                RuleContext::Default,
                "external-ref-unresolved".to_owned(),
                None,
                signers,
                policies,
            );
            ...
            let case = format!(
                "kind={contract_kind}, accept_mutable_verifier={accept_mutable_verifier}, \
                 accept_unknown_verifier={accept_unknown_verifier}"
            );
            let error = result.expect_err(&case);
            assert_eq!(
                error.wire_code(),
            ...
            assert!(
                error
                    .to_string()
                    .contains("external reference with no live tag entry"),
                "{case}: {error}"
            );
