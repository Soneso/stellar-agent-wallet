//! Adversarial fixture: spending-limit policy identification — RPC hash disagreement.
//!
//! Scenario: primary and secondary RPCs return different wasm hashes for the same
//! policy contract during `identify_spending_limit_policy`. Both hashes are
//! individually plausible allowlist candidates, but the two RPCs disagree on which
//! hash is current → `NetworkRpcDivergence`.
//!
//! This validates that the two-RPC agreement check in `identify_spending_limit_policy`
//! fires before the single-entry allowlist match — a compromised or lagging RPC
//! cannot silently substitute a policy contract identity.
...
use super::rpc_mock_helpers::{
    KNOWN_SPENDING_LIMIT_WASM_HASH, SOURCE_G, UNKNOWN_WASM_HASH, build_context_rule_scval_xdr,
...
/// Primary returns `KNOWN_SPENDING_LIMIT_WASM_HASH` for the policy; secondary
/// returns `UNKNOWN_WASM_HASH`.
///
/// The two-RPC agreement check in `identify_spending_limit_policy` detects the
/// hash disagreement and returns `NetworkRpcDivergence` before performing the
/// allowlist match.
#[tokio::test]
async fn policy_hash_rpc_disagreement_returns_rpc_divergence() {
    ...
    // Primary: serves KNOWN_SPENDING_LIMIT_WASM_HASH for the policy contract instance.
...
/// Type-level: `NetworkRpcDivergence` from spending-limit-policy identification
/// carries the same wire code as `NetworkRpcDivergence` from any other consumer.
#[test]
