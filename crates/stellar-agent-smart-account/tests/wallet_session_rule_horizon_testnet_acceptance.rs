...
//! | Fixture | Description |
//! |---------|-------------|
//! | [`h_a_install_horizon_exceeded`] | Deploy SA; invoke `wallet rules create --valid-until <oversized>` via the release binary — must exit non-zero with `validation.session_rule_horizon_exceeded` BEFORE submit |
//! | [`h_b_update_horizon_exceeded`] | Deploy SA; install a permanent rule; invoke `wallet rules set-valid-until <id> <oversized>` via the release binary — must exit non-zero with `validation.session_rule_horizon_exceeded` BEFORE submit |
//!
//! # Gating
//!
...
//! cargo test --features testnet-integration --test wallet_session_rule_horizon_testnet_acceptance
//! ```
//!
...
/// `wallet rules create --valid-until <oversized>` via the release binary.
///
...
async fn h_a_install_horizon_exceeded() {
    ...
    eprintln!(
        "[H-A] invoking binary: wallet rules create --account {} \
    ...
    let output = std::process::Command::new(&binary)
        ...
        .output()
        .expect("[H-A] spawn wallet rules create");
    ...
    assert_ne!(
        ...
        0,
        "[H-A] wallet rules create must exit non-zero when valid_until exceeds horizon; \
         stdout: {stdout}"
    );
...
/// invoke `wallet rules set-valid-until <id> <oversized>` via the release binary.
///
/// The CLI must exit non-zero with `validation.session_rule_horizon_exceeded`
...
async fn h_b_update_horizon_exceeded() {
    ...
    eprintln!(
        "[H-B] invoking binary: wallet rules set-valid-until --account {} \
    ...
    let output = std::process::Command::new(&binary)
        ...
        .env(signer_env_var, &signer_s)
        .output()
        .expect("[H-B] spawn wallet rules set-valid-until");
    ...
    assert_ne!(
        output.status.code().unwrap_or(0),
        0,
        "[H-B] wallet rules set-valid-until must exit non-zero when valid_until \
