//! `stellar-agent smart-account list-rules` — enumerate active context rules on a smart account.
//!
...
/// Arguments for `smart-account list-rules`.
///
...
#[non_exhaustive]
#[command(
    override_usage = "stellar-agent smart-account list-rules \
...
/// One active context rule in the `smart-account list-rules` JSON envelope.
///
...
/// Top-level JSON envelope for `smart-account list-rules`.
///
...
pub async fn run(args: &ListRulesArgs) -> i32 {
    ...
    info!(
        ...
        "smart-account list-rules: enumeration complete"
    );
