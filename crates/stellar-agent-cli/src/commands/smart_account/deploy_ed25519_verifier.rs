//! `stellar-agent smart-account deploy-ed25519-verifier` subcommand.
//!
//! Deploys the vendored OZ Ed25519-verifier contract WASM to a Stellar network and
...
//! # Idempotency
//!
//! If the registry already contains an Ed25519-verifier entry for the target
//! network with the same `wasm_sha256`, the command returns immediately with
//! `status: "already_deployed"` and no RPC traffic.
//!
//! This is the verifier bootstrap CLI verb for first-class Ed25519 external
//! signers (`smart-account signers add --signer-ed25519`).
...
/// Arguments for the `smart-account deploy-ed25519-verifier` subcommand.
///
...
/// Runs the `smart-account deploy-ed25519-verifier` subcommand.
///
/// Validates inputs, resolves the deployer keypair, resolves the fee, then
/// delegates to `deploy_ed25519_verifier`. Renders the result per `args.output`.
///
...
pub async fn run(args: &DeployEd25519VerifierArgs) -> i32 {
    ...
    match deploy_ed25519_verifier(deploy_args, None).await {
        Ok(result) => {
            info!(
                ...
                dry_run = args.dry_run,
                "deploy-ed25519-verifier: complete"
            );
...
fn print_success(
    result: &Ed25519VerifierDeployResult,
    envelope: &Envelope<Ed25519VerifierDeployResult>,
    format: OutputFormat,
) {
    match format {
        OutputFormat::Table => {
            ...
            #[allow(clippy::print_stdout, reason = "CLI binary intentional user output")]
            {
                ...
                println!("Ed25519 verifier {}: {}", result.status, verifier);
...
mod tests {
    ...
    const TEST_DEPLOYER_ENV_VAR: &str = "__STELLAR_AGENT_TEST_DEPLOY_ED25519_SKEY";
    ...
    /// registry write) and returns exit code 0.
    #[tokio::test]
    #[serial]
