//! `stellar-agent wallet rules` — context-rule lifecycle subcommands.
//!
//! CLI surface for the
...
//! # Subcommands
//!
//! - [`CreateArgs`] — `wallet rules create` (OZ `add_context_rule`)
//! - [`GetArgs`] — `wallet rules get <id>` (OZ `get_context_rule`, read-only)
//! - [`SetNameArgs`] — `wallet rules set-name <id> <name>`
//!   (OZ `update_context_rule_name`)
//! - [`SetValidUntilArgs`] — `wallet rules set-valid-until <id> <ledger | none>`
//!   (OZ `update_context_rule_valid_until`; `none` clears expiry → permanent)
//! - [`DeleteArgs`] — `wallet rules delete <id>` (OZ `remove_context_rule`)
//! - [`AddPolicyArgs`] — `wallet rules add-policy` (OZ `add_policy`).
//! - [`RemovePolicyArgs`] — `wallet rules remove-policy` (OZ `remove_policy`).
//! - [`ListArgs`] — **`wallet rules list`** (canonical name;
//!   delegates to [`crate::commands::wallet::sa::list_rules::run`]).
//!   `wallet sa list-rules` is retained as an alias for backwards-compatibility
//!   (no deprecation warning is emitted).
//!
//! # Command-name aliasing
//!
//! `wallet rules list` is the canonical name. `wallet sa list-rules` is retained
...
//!
//! A `tracing::info!` row carrying `command = "wallet rules list"` is emitted
...
use crate::commands::wallet::common::{CommonArgsView, CommonHandlerContext, SignerSourceFlags};
use crate::commands::wallet::sa::list_rules as sa_list_rules;
...
/// Arguments for the `wallet rules` subcommand group.
#[derive(Debug, Args)]
#[non_exhaustive]
...
/// Subcommands of `stellar-agent wallet rules`.
...
pub enum RulesSubcommand {
    ...
    ///
    /// **Canonical command:** `wallet rules list`. Delegates directly to
    /// [`crate::commands::wallet::sa::list_rules::run`]; argument shape and
    /// JSON envelope are identical to `wallet sa list-rules`.
    ///
    /// `wallet sa list-rules` is retained unchanged as a secondary entry point
    /// for backwards-compat. No deprecation warning is emitted.
    ///
    /// A `tracing::info!` row carrying `command = "wallet rules list"` is
...
/// Arguments for `wallet rules list`.
///
...
///
/// The JSON envelope shape is identical to `wallet sa list-rules`.
pub type ListArgs = sa_list_rules::ListRulesArgs;

/// Runs the `wallet rules` subcommand group.
///
/// # Errors
///
...
/// Dispatch handler for `wallet rules list`.
///
/// Emits a structured `tracing::info!` row carrying the operator-invoked
/// command name (`"wallet rules list"`) before delegating to the shared
/// `wallet sa list-rules` handler.  The log row allows forensic correlation
/// across the audit log: an operator invoking `wallet rules list` produces
/// `command = "wallet rules list"`, whereas `wallet sa list-rules` produces
/// `command = "wallet sa list-rules"` at its own dispatch site — both refer
...
/// Arguments for `wallet rules create`.
///
...
pub struct CreateArgs {
    ...
    /// the target network passphrase (written by `wallet sa deploy-webauthn-verifier`).
    ///
    ...
    ///
    /// By default (`false`), `wallet rules create` fails with
...
/// Result envelope for `wallet rules create`.
///
...
async fn create_run(args: &CreateArgs) -> i32 {
    ...
    // `wallet rules create` does not currently support --policy flags; if/when
    ...
    if !args.signer_webauthn.is_empty() {
        ...
        // this network (written by `wallet sa deploy-webauthn-verifier`).
                    ...
                    &WalletError::Validation(ValidationError::AddressInvalid {
                        input: format!(
                            "no WebAuthn verifier deployed for network '{}'; \
                                 run: wallet sa deploy-webauthn-verifier",
                            network_passphrase
                        ),
                    }),
                    args.common.output,
...
/// Arguments for `wallet rules get`.
#[non_exhaustive]
#[derive(Debug, Args)]
pub struct GetArgs {
...
/// Result envelope for `wallet rules get`. `present == false` means the
...
// ─────────────────────────────────────────────────────────────────────────────
// `wallet rules set-name`
// ─────────────────────────────────────────────────────────────────────────────

/// Arguments for `wallet rules set-name`.
#[non_exhaustive]
#[derive(Debug, Args)]
...
/// Result envelope for `wallet rules set-name`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
// ─────────────────────────────────────────────────────────────────────────────
// `wallet rules set-valid-until`
// ─────────────────────────────────────────────────────────────────────────────

/// Arguments for `wallet rules set-valid-until`.
#[non_exhaustive]
#[derive(Debug, Args)]
...
/// Result envelope for `wallet rules set-valid-until`.
#[derive(Debug, Clone, Serialize, Deserialize)]
        ...
        .await
    {
        Ok(()) => {
            ...
            info!(
                ...
                "wallet rules set-valid-until: updated",
            );
...
/// Arguments for `wallet rules delete`.
#[non_exhaustive]
#[derive(Debug, Args)]
...
/// Result envelope for `wallet rules delete`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
// ─────────────────────────────────────────────────────────────────────────────
// `wallet rules verify-pins`
// ─────────────────────────────────────────────────────────────────────────────

/// Arguments for `wallet rules verify-pins`.
///
...
/// Result envelope for `wallet rules verify-pins`.
///
...
async fn verify_pins_run(args: &VerifyPinsArgs) -> i32 {
    ...
    info!(
        smart_account = %account_redacted,
        rule_id = args.rule_id,
        "wallet rules verify-pins: running",
    );
        ...
        .await
    {
        Ok(sa_result) => {
            ...
            info!(
                ...
                "wallet rules verify-pins: completed",
            );
...
// ─────────────────────────────────────────────────────────────────────────────
// `wallet rules add-policy`
// ─────────────────────────────────────────────────────────────────────────────

/// Arguments for `wallet rules add-policy`.
///
...
/// Result envelope for `wallet rules add-policy`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
async fn add_policy_run(args: &AddPolicyArgs) -> i32 {
    ...
    // should re-fetch via `wallet rules get` and retry — or pivot to the
        ...
        .await
    {
        Ok(policy_id) => {
            info!(
                ...
                rule_id = args.rule_id,
                policy_id,
                "wallet rules add-policy: installed",
            );
...
// ─────────────────────────────────────────────────────────────────────────────
// `wallet rules remove-policy`
// ─────────────────────────────────────────────────────────────────────────────

/// Arguments for `wallet rules remove-policy`.
///
...
/// Result envelope for `wallet rules remove-policy`.
#[derive(Debug, Clone, Serialize, Deserialize)]
        ...
        .await
    {
        Ok(()) => {
            info!(
                ...
                "wallet rules remove-policy: removed",
            );
...
///
/// Used only by `wallet rules get` which is a simulation-only read path.
...
/// human-render is provided by the `wallet rules list` enumerator.
...
mod tests {
    ...
    // ── Per-rule signer cap at `wallet rules create` ─────────────────────────
    //
    ...
    // ── Per-rule policy cap at `wallet rules add-policy` ──────────────────────
    //
    ...
    // ── wallet rules list alias delegation ────────────────────────────────────
    //
    // `wallet rules list` and `wallet sa list-rules` produce identical JSON
    ...
    // `command = "wallet rules list"` BEFORE delegating to `sa_list_rules::run`.
    ...
    ///
    /// Both `wallet rules list` and `wallet sa list-rules` share the same
    ...
    fn wallet_rules_list_and_sa_list_rules_emit_byte_identical_json() {
        ...
        // Serialise via the `wallet rules list` path (delegates to sa_list_rules).
        ...
        assert_eq!(
            ...
            "wallet rules list and wallet sa list-rules must produce identical JSON envelopes"
        );
    ...
    /// Dispatch-site log carries `command = "wallet rules list"`.
    ///
    /// The structured-log row emitted at the `list_rules_run` dispatch site must
    /// carry `command = "wallet rules list"` verbatim so audit-log forensic
    /// correlation can distinguish the operator-invoked command from the
    /// `wallet sa list-rules` alias.
    ///
    ...
    fn list_rules_dispatch_log_carries_command_name() {
        ...
        assert!(
            log_output.contains("wallet rules list"),
            "dispatch-site log must contain 'wallet rules list'; captured: {log_output}"
        );
        // The message must NOT contain "wallet sa list-rules" — that is emitted
        // by `sa::list_rules::run` at its own dispatch site (distinct event).
        // (No assertion needed for the positive case on the sa side — that is
    ...
    /// `wallet rules create` parse-test:
    ///
    ...
    /// `wallet rules set-valid-until` parse-test:
    ///
    ...
    /// `wallet rules delete` parse-test:
    ///
    /// - `--rule-id` is required
    /// - `--auth-rule-id` is optional
    ...
    /// `wallet rules set-name` parse-test:
    ///
