//! `stellar-agent wallet sa timelock` subcommand group — OZ upgrade timelock.
//!
...
//! enum. The parent [`super::run`] routes `SaSubcommand::Timelock(args)` to
...
/// Arguments for the `wallet sa timelock` subcommand group.
#[derive(Debug, Args)]
#[non_exhaustive]
...
/// Subcommands of `stellar-agent wallet sa timelock`.
...
/// Runs the `wallet sa timelock` subcommand group.
///
