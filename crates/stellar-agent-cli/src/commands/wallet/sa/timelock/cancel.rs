//! `stellar-agent wallet sa timelock cancel` — cancel a pending timelock operation.
//!
...
/// Arguments for `wallet sa timelock cancel`.
#[derive(Debug, Args)]
#[non_exhaustive]
#[command(
    override_usage = "stellar-agent wallet sa timelock cancel \
...
/// Runs `wallet sa timelock cancel`.
///
...
pub async fn run(args: &CancelArgs) -> i32 {
    ...
    if secondary_rpc_url == args.rpc_url {
        info!(
            ...
            rpc_host = %rpc_host,
            "wallet sa timelock cancel: --secondary-rpc-url not set or equals \
    ...
    info!(
        ...
        request_id = %request_id,
        "wallet sa timelock cancel: submitting"
    );
    ...
    info!(
        operation_id = %operation_id.redacted(),
        request_id = %request_id,
        "wallet sa timelock cancel: confirmed on-chain"
    );
