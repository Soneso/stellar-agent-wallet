//! `stellar-agent wallet sa timelock schedule` — schedule a timelock operation.
//!
...
/// Arguments for `wallet sa timelock schedule`.
#[derive(Debug, Args)]
#[non_exhaustive]
#[command(
    override_usage = "stellar-agent wallet sa timelock schedule \
...
/// Runs `wallet sa timelock schedule`.
///
...
pub async fn run(args: &ScheduleArgs) -> i32 {
    ...
    if secondary_rpc_url == args.rpc_url {
        info!(
            ...
            rpc_host = %rpc_host,
            "wallet sa timelock schedule: --secondary-rpc-url not set or equals \
    ...
    info!(
        ...
        request_id = %request_id,
        "wallet sa timelock schedule: submitting"
    );
    ...
    info!(
        ...
        request_id = %request_id,
        "wallet sa timelock schedule: confirmed on-chain"
    );
