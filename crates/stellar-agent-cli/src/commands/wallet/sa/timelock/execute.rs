//! `stellar-agent wallet sa timelock execute` — execute a ready timelock operation.
//!
...
/// Arguments for `wallet sa timelock execute`.
#[derive(Debug, Args)]
#[non_exhaustive]
#[command(
    override_usage = "stellar-agent wallet sa timelock execute \
...
/// Runs `wallet sa timelock execute`.
///
...
pub async fn run(args: &ExecuteArgs) -> i32 {
    ...
    if secondary_rpc_url == args.rpc_url {
        info!(
            ...
            rpc_host = %rpc_host,
            "wallet sa timelock execute: --secondary-rpc-url not set or equals \
    ...
    info!(
        ...
        request_id = %request_id,
        "wallet sa timelock execute: submitting"
    );
    ...
    info!(
        ...
        request_id = %request_id,
        "wallet sa timelock execute: confirmed on-chain"
    );
