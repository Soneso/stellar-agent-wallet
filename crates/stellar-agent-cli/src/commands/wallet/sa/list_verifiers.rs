//! `stellar-agent wallet sa list-verifiers` — enumerate the verifier allowlist.
//!
...
/// Arguments for `wallet sa list-verifiers`.
#[derive(Debug, Args)]
#[non_exhaustive]
#[command(
    override_usage = "stellar-agent wallet sa list-verifiers [--output {json|table}]",
...
/// Top-level JSON envelope for `wallet sa list-verifiers`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[non_exhaustive]
...
/// Runs the `wallet sa list-verifiers` subcommand.
///
