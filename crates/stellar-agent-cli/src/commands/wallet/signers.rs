//! `stellar-agent wallet signers` — signer-set lifecycle subcommands.
//!
//! CLI surface for
...
//! # Subcommands
//!
//! - [`ListArgs`] — `wallet signers list` — reads on-chain signer set; writes
//!   a `SaSignerSetBaselined` audit row if no prior baseline exists.
//! - [`RefreshArgs`] — `wallet signers refresh` — unconditionally writes a new
//!   `SaSignerSetBaselined` row (programmatic re-anchor after intentional
//!   out-of-band mutation).
//! - [`AddArgs`] — `wallet signers add` — adds one signer to a context rule via
...
//! - [`RemoveArgs`] — `wallet signers remove` — removes one signer by `signer_id`
...
//! - [`SetThresholdArgs`] — `wallet signers set-threshold` — changes the
//!   signing threshold via OZ `ThresholdPolicyContract::set_threshold`; emits
//!   `SaThresholdChanged`.
//!
//! # Signer-source modes (mirror of `wallet rules`)
//!
...
/// Arguments for the `wallet signers` subcommand group.
#[derive(Debug, Args)]
#[non_exhaustive]
...
/// Subcommands of `stellar-agent wallet signers`.
...
/// Runs the `wallet signers` subcommand group.
///
/// # Errors
///
...
/// Arguments for `wallet signers list`.
///
...
#[derive(Debug, Args)]
#[command(
    override_usage = "stellar-agent wallet signers list [OPTIONS] --account <C_STRKEY> --rule-id <U32>"
)]
pub struct ListArgs {
...
/// Result envelope for `wallet signers list`.
///
...
async fn list_run(args: &ListArgs) -> i32 {
    ...
    info!(
        ...
        "wallet signers list: querying on-chain signer set"
    );
...
/// Arguments for `wallet signers refresh`.
///
...
/// Result envelope for `wallet signers refresh`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
async fn refresh_run(args: &RefreshArgs) -> i32 {
    ...
    info!(
        rule_id = args.rule_id,
        account = %redact_strkey_first5_last5(&args.account),
        "wallet signers refresh: writing fresh baseline"
    );
...
/// Arguments for `wallet signers add`.
///
...
pub struct AddArgs {
    ...
    /// the target network (written by `wallet sa deploy-webauthn-verifier`).
...
/// Result envelope for `wallet signers add`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
async fn add_run(args: &AddArgs) -> i32 {
    ...
    // `[OZ:TooManySigners]`. Retry via `wallet rules get` + re-submit.
    let source_account_strkey = match ctx.signer.public_key().await {
        Ok(pk) => pk.to_string(),
        Err(e) => {
            return emit_error(
    ...
    info!(
        ...
        "wallet signers add: submitting add_signer"
    );
...
/// Arguments for `wallet signers remove`.
///
/// Removes a signer by its on-chain `signer_id`.  Use `wallet signers list`
...
pub struct RemoveArgs {
    ...
    /// On-chain signer ID to remove (from `wallet signers list`).
...
/// Result envelope for `wallet signers remove`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
async fn remove_run(args: &RemoveArgs) -> i32 {
    ...
    info!(
        ...
        account = %redact_strkey_first5_last5(&args.account),
        "wallet signers remove: submitting remove_signer"
    );
...
/// Arguments for `wallet signers set-threshold`.
///
...
/// Result envelope for `wallet signers set-threshold`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
async fn set_threshold_run(args: &SetThresholdArgs) -> i32 {
    ...
    info!(
        ...
        account = %redact_strkey_first5_last5(&args.account),
        "wallet signers set-threshold: submitting set_threshold"
    );
...
mod tests {
    ...
    // ── Per-rule signer cap at `wallet signers add` ──────────────────────────
    //
