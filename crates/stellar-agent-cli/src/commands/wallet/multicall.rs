//! `stellar-agent wallet multicall` subcommand.
//!
...
/// Arguments for `wallet multicall`.
///
...
/// Runs the `wallet multicall` subcommand.
///
...
pub async fn run(args: &MulticallArgs) -> i32 {
    ...
    let secondary_rpc_url = {
        ...
        let from_profile =
            match stellar_agent_core::profile::loader::load(&profile_name, Some(&registry)) {
                ...
                Err(e) => {
                    warn!(
                        ...
                        error = %e,
                        "wallet multicall: non-fatal profile load error (secondary_rpc_url not \
        ...
        Some(ClassicFeeChoice::Auto(_)) => {
            return Err(WalletError::Validation(
                ValidationError::UnsupportedFeeMode {
                    mode: "auto".to_owned(),
                    reason: "wallet multicall rejects auto fee mode during CLI parsing".to_owned(),
                },
            ));
        }
    };
...
///
/// The CLI multicall path (`wallet multicall`) bypasses profile-configured
...
async fn resolve_signer(
    ...
    let var_name = flags.signer_secret_env.as_deref().ok_or_else(|| {
        WalletError::Auth(AuthError::KeyringNotFound {
            name: "no signer-source flag specified for wallet multicall; \
