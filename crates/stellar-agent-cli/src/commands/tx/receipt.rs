...
//!
//! Both hold the operator's spending caps until someone decides. This verb is
//! that decision.
//!
...
async fn run_clear(args: &ClearArgs) -> i32 {
    ...
    let existing = match receipts.get(&args.envelope_hash) {
        ...
        Ok(None) => {
            render_json(&Envelope::<()>::err_raw(
                "submission.record_unavailable",
                "no submission record exists for this envelope hash",
            ));
            return 1;
        }
        Err(e) => {
    ...
    let chain = match client.get_transaction_status(&existing.tx_hash).await {
        Ok(s) => s,
        Err(e) => {
            render_json(&Envelope::<()>::err_raw(
                "submission.not_clearable",
    ...
    if chain.status == "SUCCESS" || chain.status == "FAILED" {
        render_json(&Envelope::<()>::err_raw(
            ...
            format!(
                ...
                stellar_agent_network::redact_tx_hash(&existing.tx_hash),
            ),
        ));
        return 1;
    }
    ...
    // The receipt is marked last. It is the record the other two are keyed to,
    // and marking it first would leave a reservation open under a receipt no
    // verb will accept again: a re-run refuses `submission.not_clearable` and
    // only a later sequence-consumed pass would free the hold.
    ...
    if !already_recorded && let Some(audit) = audit_writer.as_ref() {
